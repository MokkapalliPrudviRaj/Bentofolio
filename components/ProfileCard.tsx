...
export const ProfileCard: React.FC<{ className?: string }> = ({
  className,
}) => {
  ...
  return (
    ...
    >
      ...
      {/* Profile image */}
      <div className="absolute bottom-0 right-0 w-[80%] h-full sm:w-[65%] md:w-[55%] z-0 pointer-events-none select-none">
        <img
          src={imgSrc}
          onError={() =>
            setImgSrc(
      ...
      <div className="flex flex-col justify-between h-full p-5 sm:p-6 md:p-10 relative z-10">
        <div className="space-y-6 w-full sm:max-w-[70%] lg:max-w-[55%]">
          <Sparkles
          ...
          {/* Title */}
          <h1 className="text-4xl md:text-5xl lg:text-6xl font-black tracking-tighter text-neutral-900 dark:text-white leading-[0.9] break-words">
            PRUDVI
            <br />
            RAJ.
          </h1>
          <p className="mt-3 sm:mt-4 text-base sm:text-lg md:text-xl font-medium text-neutral-500 dark:text-neutral-400 leading-relaxed">
            Angular Developer & UI/UX Specialist crafting high-performance
